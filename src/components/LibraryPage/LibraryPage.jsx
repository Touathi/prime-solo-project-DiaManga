...
function InfoPage() {
  ...
  const mangaLibrary = useSelector(
    (store) => store.setMangaLibrarys.setMangaLibrary
  );
  ...
  const handleDelete = (manga) => {
    ...
    dispatch({
      type: "DELETE_MANGA_BOOK",
      payload: { id: manga.id, mangaid: manga.manga_id },
    });
    dispatch({ type: "GET_MANGA_LIBRARY" });
  };
                ...
                <div>
                  <div key={i} className="mangaBook">
                    <div onClick={() => handleClick(manga)}>
                      <Link
                        to={`/library/mangadetails/${manga.manga_id}/${manga.title}`}
                      >
                        <img
                          src={manga.img}
                          alt="manga_pic"
                      ...
                      <div className="Libtitle">{manga.title}</div>
                    </div>
                  </div>
                </div>
              ) : (
                <div className="LibContainer" key={i}>
                  ...
                  <div className="mangaBook">
                    ...
                    <div onClick={() => handleClick(manga)}>
                      ...
                      <div className="LibtitleBefore">{manga.title}</div>
                    </div>
                  </div>
                </div>
              )
